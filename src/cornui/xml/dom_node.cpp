...
namespace cornui {
    ...
    void DOMNode::onClick() {
        if (!this->attributes_.contains("onclick")) return;
        duk_context* ctx = this->dom_->getUI().getJSRuntime()->getImpl()->ctx_;

        // Compile the "onclick" function
        std::string jsCode = this->attributes_.at("onclick");
        ...
        // Call the "onclick" function with the current DOMNode as "this"
        duk_pcall_method(ctx, 0);

        // Pop the result or error generated by duk_pcall_method
        duk_pop(ctx);
    }
}
